...
export default function DownloadModal({ fileName, onDownload, onClose }: DownloadModalProps) {
  ...
  // Sync state if ad settings change while modal is open
  if (adBeforeDownload && secondsLeft === 0 && !canDownload) {
  ...
  // Focus download button when ready
  useEffect(() => {
    if (canDownload) {
  ...
  useEffect(() => {
    if (!adBeforeDownload || canDownload) return;
  ...
  }, [adBeforeDownload, adDuration, canDownload]);
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <div id={titleId} className="flex items-center gap-2 font-semibold text-slate-800">
        ...
        <div className="px-6 py-6 flex flex-col items-center gap-4">
          {adBeforeDownload && !canDownload ? (
            ...
            <div className="flex flex-col items-center gap-3 py-2">
              <div className="w-16 h-16 rounded-full bg-green-100 flex items-center justify-center animate-bounce shadow-sm">
                <Download className="w-8 h-8 text-green-600" />
              </div>
              <p className="text-slate-700 font-medium text-center" aria-live="polite">
                Ready to download!
              </p>
